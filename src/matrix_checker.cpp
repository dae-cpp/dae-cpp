...
int TimeIntegrator::m_matrix_checker(sparse_matrix_holder &A, MKL_INT size)
{
    ...
    case MKL_SPARSE_CHECKER_NON_MONOTONIC:
        std::cout
            << "The input array is not 0 or 1 based (ia[0] is not 0 or 1) or "
               "elements of ia are not in non-decreasing order as required.\n";
        break;
    case MKL_SPARSE_CHECKER_OUT_OF_RANGE:
        std::cout
            << "The value of the ja array is lower than the number of the "
               "first column or greater than the number of the last column.\n";
        break;
    case MKL_SPARSE_CHECKER_NONORDERED:
        std::cout << "The elements of the ja array are not in non-decreasing "
                     "order in each row as required.\n";
        break;
    default:
    ...
    std::cout << "Matrix check details: (" << pt.check_result[0] << ", "
